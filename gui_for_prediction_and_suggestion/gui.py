...
class Application(tk.Frame):
    ...
    def predict(self):
        self.xT = np.array([0 for i in range(120)])
        ...
        res = self.cls.predict_proba([self.xT])
        self.predict_text.set('Win rate: ' + str(res[0][1]))
    ...
    def suggest(self):
        ...
        res = self.cls.predict_proba(heroes_arr)
        win_rate = []
        for i in range(len(hero_arr)):
            win_rate.append({'hero': hero_arr[i], 'win_rate': res[i][1]})
